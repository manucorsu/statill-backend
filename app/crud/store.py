...
def create(store_data: StoreCreate, session: Session):
    ...
    if (store_data.ps_max and not store_data.ps_value) or (
        not store_data.ps_max and store_data.ps_value
    ):
        raise HTTPException(
            400,
            "Points-related fields should either both have a value or both be None (but not one each)",
        )
